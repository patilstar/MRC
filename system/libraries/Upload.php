...
class CI_Upload {
	...
	// --------------------------------------------------------------------
         ...
         * @return      bool
         */
        function do_upload_files($field = 'userfile', $idx = "0")
        {
                // Is $_FILES[$field] set? If not, no reason to continue.
                if ( ! isset($_FILES[$field]))
                {
		...
		// Was the file able to be uploaded? If not, determine the reason why.
                if ( ! is_uploaded_file($_FILES[$field]['tmp_name'][$idx]))
                {
                        $error = ( ! isset($_FILES[$field]['error'][$idx])) ? 4 : $_FILES[$field]['error'][$idx];
		...
		// Set the uploaded data as class variables
                $this->file_temp = $_FILES[$field]['tmp_name'][$idx];
                $this->file_name = $this->_prep_filename($_FILES[$field]['name'][$idx]);
                $this->file_size = $_FILES[$field]['size'][$idx];
                $this->file_type = preg_replace("/^(.+?);.*$/", "\\1", $_FILES[$field]['type'][$idx]);
                $this->file_type = strtolower($this->file_type);
                $this->file_ext  = $this->get_extension($_FILES[$field]['name'][$idx]);
